...
export default function TrialBalance({ filters, trialBalance: data }) {
    const balanced = Math.abs(data.totalDebit - data.totalCredit) < 0.01;

    return (
        <AppLayout title="Trial Balance">
            <Head title="Trial Balance" />
            <PeriodeFilter route="/trial-balance" filters={filters} />

            <ReportCard title="Trial Balance" subtitle="Ringkasan saldo per klasifikasi akun" badge={<BalanceBadge balanced={balanced} />}>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead className="bg-gray-50 text-gray-600">
                            <tr>
                                <th className="text-left py-2.5 px-4 font-medium w-24">Kode</th>
                                <th className="text-left py-2.5 px-4 font-medium">Klasifikasi</th>
                        ...
                        <tbody>
                            {data.rows.map((r) => (
                                <tr key={r.code} className="border-t border-gray-100 hover:bg-gray-50">
                                    ...
                                    <td className="py-2.5 px-4 text-right tabular-nums">{fmt(r.debit)}</td>
                                    <td className="py-2.5 px-4 text-right tabular-nums">{fmt(r.credit)}</td>
                                </tr>
                            ))}
                            {data.rows.length === 0 && (
